...
注意，前面公式中涉及的渐近方差是很尴尬的。一种建议是通过通常的插件规则$F_{N}(y|x)$分别用$F_{H}(y|x)$和$\widehat{F}_{H}(y|x)$和$ \widehat{G}_{H}(z|x)$，或者用复杂设计的重采样方法（cfr.[3]，[13]）来构造估计量$ \widehat{V}_{x}$。显然，对于无条件的不确定性测量（4.33），使用（4.60）也有类似的考虑。
